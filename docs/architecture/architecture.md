...
Кратко: консольный UI и `session` не зависят от gRPC; обмен идёт через интерфейс `internal/transport.Transport`; реализация — `internal/grpcchat` (bidirectional streaming `StreamChat`).

Диаграммы и полные требования можно добавить в этот файл по мере развития курсового проекта.
